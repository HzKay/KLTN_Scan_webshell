...
class Signatures
{
    ...
    public function getAll()
    {
        ...
        if (empty($optimized)) {
            ...
            $this->optimizeSignatures($optimized);
        }
    ...
    //  * @param $sigs
     */
    public function optimizeSignatures(&$sigs)
    {
        ...
        $this->optimizeSignaturesCheck($sigs);
        ...
        for ($i = 24; $i >= 1; ($i > 4) ? $i -= 4 : --$i) {
            $txt = preg_replace_callback('#^((?>(?:\\\\.|\\[.+?\\]|[^(\n]|\((?:\\\\.|[^)(\n])++\))(?:[*?+]\+?|\{\d+(?:,\d*)?\}[+?]?|)){' . $i . ',})[^\n]*+(?:\\n\\1(?![{?*+]).+)+#im', [$this, 'optimizeSignatureMergePrefixes'], $txt);
        }
    ...
    //  * @return string
     */
    protected function optimizeSignatureMergePrefixes($m)
    {
        $limit = 8000;
    ...
    //  * @return bool
     */
    protected function optimizeSignaturesCheck(&$signs)
    {
        $result = true;
