<?php
    class clsScan 
    {
        ...
        public function validFileContent ($file)
        {
            ...
            foreach ($this->sig_normal as $keyword)
            {
            ...
            // foreach ($this->sig_reg as $signature)
            // {
            //     $pattern = preg_quote($signature, '#');
            //     $pattern = str_replace([' ', "'", '"'], ['[\s]*', '[\'"]', '[\'"]'], $pattern);
            //     preg_match_all($pattern, $fileContent, $output, PREG_SET_ORDER);
            //     var_dump($output);
            // }
        ...
        {
            $AllFile = $this->getFullPath($filePath);
            $this->importToCSV("", "", true);
            foreach ($AllFile as $file)
            {
                echo $this->importToCSV($file, $this->createHashCode($file)) . "<br>";
            }
        }
        ...
        public function quaranFile($fileList, $action)
        {
            ...
            for ($index = 0; $index < count($fileList); $index++)
            {
                if ($action[$index] == 1)
                {
                    ...
                    $filePath = $this->path . $fileList[$index];
